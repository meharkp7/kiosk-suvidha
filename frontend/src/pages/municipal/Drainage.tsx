...
export default function Drainage() {
      ...
      <KioskLayout title="✅ Reported" showHeader={true} showNav={true}>
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-green-50 rounded-xl p-8">
            ...
            <p className="text-green-600 mt-2">Complaint ID: DG{Date.now()}</p>
  ...
  return (
    <KioskLayout title="🚰 Drainage Issue" subtitle="Report drainage blockage" showHeader={true} showNav={true} onBack={() => navigate("/services-dashboard")}>
      <div className="max-w-2xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <h3 className="text-lg font-semibold mb-4">Report Drainage Problem</h3>
            <div className="space-y-4">
              <select value={issueType} onChange={(e) => setIssueType(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg" required>
                ...
                <option value="blockage">Drain Blockage</option>
                <option value="overflow">Drain Overflow</option>
                <option value="broken">Broken/Collapsed Drain</option>
              ...
              <select value={severity} onChange={(e) => setSeverity(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg" required>
